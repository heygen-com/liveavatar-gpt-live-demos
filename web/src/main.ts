...
const homePlayer = document.getElementById("home-player") as HTMLElement;
...
const setStage = (state: "idle" | "connecting" | "live") => {
  ...
  // The resting frame (public/overlays/home.html) starts with the first video
  // frame, so its intro plays as the avatar fades in, and is torn down with
  // the session. Setting the src only here (not at page load) is what keeps
  // the intro from having already finished before anyone sees it.
  if (state === "live") {
    homePlayer.setAttribute("src", `/overlays/home.html?t=${Date.now()}`);
  } else {
    homePlayer.removeAttribute("src");
  }
};
...
function drawMicWave(): void {
  ...
  if (live) {
    ...
    grad.addColorStop(0, "#33d1ff");
    grad.addColorStop(1, "#00c3ff");
    waveCtx.fillStyle = grad;
  } else {
    waveCtx.fillStyle = "#b9c0c7";
  }
  const barW = 6;
...
//   window.__ui({ widget: "pricing_sheet", props: { title: "LiveAvatar pricing", rows: [{ plan: "FULL Mode", price: "2 credits / minute" }] } })
// and window.__home() to preview the resting frame without an avatar.
(window as unknown as { __ui: typeof overlays.render }).__ui = overlays.render;
(window as unknown as { __home: () => void }).__home = () => setStage("live");
