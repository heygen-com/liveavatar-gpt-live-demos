...
server/     Orchestrator. Node + ws only. `pnpm dev` = tsx watch.
            prompts/*.md is the persona (heads-up poker coach) — the intended
            customization point. src/prompts.ts loads it and holds the
            non-negotiable delegation/tool mechanics prompts. src/poker.ts is
            the game simulator — the single source of game truth.
web/        Browser client. Vite + livekit-client + @hyperframes/player.
```

`pnpm dev` at the root runs both. Vite (:5175) proxies `/api` and `/ws` to the
server (:8789), so the browser sees one origin — ports are +2 from the
language demo's (8787/5173) so both can run side by side. Config is one `.env` at the
...
- **Turns are projected, not received.** v3 has no turn events, only timed
  ...
  an assistant turn *closing* is the turn-boundary hook. Tune the gap, don't
...
- **Staging is per-widget, client-decided** (`web/src/overlays/index.ts`) —
  never a tool argument. The poker table sets PiP (avatar bottom-right); its
  composition keeps that quadrant clear.
- **Game state is server state.** Cards, pot, stacks, and outcomes live only
  in the simulator (`server/src/poker.ts`). The model relays the player's
  action and narrates the tool result — never let it supply cards or outcomes;
  it will misremember them. The table renders the simulator's snapshot, whole,
  on every change (`reveal` names what animates).
...
`server/prompts/*.md` re-skins the persona only. The poker domain logic —
the simulator (`server/src/poker.ts`), the game-loop directives in
`src/prompts.ts`, the deal fallback in `session.ts`, the taken tool-key sets
(`["action"]`, `["ready"]`) — lives in code and is the adopter's to replace.
[docs/REPURPOSING.md](docs/REPURPOSING.md) is the excavation checklist: every
poker hook by file and symbol, the taken key sets, and the server-owned-state
pattern (`PokerGame` → `onToolCall` → `pushTable`) to copy for a new domain.
