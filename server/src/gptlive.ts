...
export interface GptLiveEvents {
   ...
   * Shape by convention: {shown: boolean, error?, rows?} —
   * show_pricing_sheet returns the exact rows placed on the sheet so the
...
export class GptLiveBridge {
  ...
  private startSession(): void {
    ...
    // the directive is mechanics and always appended. Everything at startup:
    // instructions are the one channel the live model reliably follows, and
    // mid-session appends cut off whatever loop the model was running.
    // Persona, then the product facts it answers from, then the mechanics.
    // KNOWLEDGE goes to the delegated model too (prompts.ts) — one file, both
    // models, so they cannot contradict each other mid-conversation.
    const instructions = `${DEFAULT_INSTRUCTIONS}\n\n${KNOWLEDGE}${LIVE_DIRECTIVE}`;
