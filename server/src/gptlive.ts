...
export interface GptLiveEvents {
  ...
  /**
   * One tool call. The returned object IS the function-call
   * result sent back to the API (JSON-stringified) — every actionable call
   * must get exactly one, or it stays pending server-side and blocks all
   * later delegations. Shape by convention: {ok: boolean, error?, happened?,
   * state?} — game moves return what happened at the table so the backend can
   * narrate the authoritative outcome instead of guessing.
   */
  onToolCall(
    name: string | null,
...
export class GptLiveBridge {
  ...
  private startSession(): void {
    ...
    // the directive is mechanics and always appended.
    // Persona + delegation mechanics + the whole game loop, all at startup:
    // instructions are the one channel the live model reliably follows, and
    // mid-session appends broke the predecessor demo's practice pacing.
    const instructions =
      DEFAULT_INSTRUCTIONS + LIVE_DIRECTIVE + GAME_DIRECTIVE;
  ...
  private handleToolCall(item: Json): void {
    ...
    if (typeof item.arguments === "string" && item.arguments) {
      try {
        ...
        result = {
          ok: false,
          error: "arguments were not valid JSON; nothing happened",
        };
      }
    } else {
      result = { ok: false, error: "no arguments; nothing happened" };
    }
    ...
    this.log(
      `gptlive tool result sent (${name ?? "unnamed"} → ok=${result.ok})`,
    );
  }
